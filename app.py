...
def registra_log(targa, azione, d, u):
    try:
        # Recupero numero chiave per arricchire il log
        n_chiave = 0
...
if st.session_state['user_autenticato'] is None:
    ...
    # --- 13. VERIFICA ZONE ---
        ...
        if res.data:
            ...
            st.dataframe(
                df_zona[["targa", "marca_modello", "colore", "numero_chiave"]],
    ...
    # --- 14. DASHBOARD ZONE ---
        ...
        if res.data:
            ...
            st.dataframe(
                df[["Data/Ora", "targa", "azione", "utente", "numero_chiave"]], 
                use_container_width=True
            )
    ...
    # --- 15. LOG ---
        ...
        if logs.data:
            ...
            if not df.empty:
                ...
                c3.metric("👥 Operatori coinvolti", kpi_ops["utente"].nunique())
                with st.expander("📋 Dettaglio movimenti per operatore"):
                    st.dataframe(kpi_ops, use_container_width=True)

            # 📊 1️⃣ KPI PER ZONA (con numero chiave)
            st.markdown("### 📍 KPI Zone")

            if {"dettaglio", "numero_chiave"}.issubset(df.columns):
                df_zone_kpi = df.copy()
                df_zone_kpi["zona"] = df_zone_kpi["dettaglio"].str.replace("In ", "", regex=False)

                kpi_zone = (
                    df_zone_kpi.groupby("zona")
                    .agg(
                        Movimenti=("targa", "count"),
                        Chiavi_coinvolte=("numero_chiave", lambda x: ", ".join(sorted({str(int(i)) for i in x if i and str(i).isdigit()})))
                    )
                    .reset_index()
                ...
                if not kpi_zone.empty:
                    ...
                    with st.expander("📋 Dettaglio movimenti per zona (con chiavi)"):
                        st.dataframe(kpi_zone, use_container_width=True)
            ...
            with pd.ExcelWriter(out_log, engine="xlsxwriter") as w:
                df_export[cols_export].to_excel(w, index=False)
