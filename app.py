...
def app_header(utente=None, ruolo=None):
    ...
    if utente and ruolo:
        badge = f"""
        <div style="background:#eff6ff; color:#1d4ed8; padding:8px 14px; border-radius:999px; font-weight:700;">
            👤 {utente} · {ruolo}
        </div>
        """
    st.markdown(f"""
    <div class="card-ui" style="padding:16px 20px;">
        <div style="display:flex; justify-content:space-between; align-items:center; flex-wrap:wrap; gap:10px;">
            <div>
                <div style="font-size:30px; font-weight:800; color:#0f172a;">🚗 Autoclub Usato 1.1</div>
                <div style="color:#475569; font-size:14px;">Gestione piazzale, movimenti, chiavi e zone</div>
            </div>
            {badge}
        </div>
    </div>
...
if 'last_action' not in st.session_state:
    st.session_state['last_action'] = datetime.now(timezone.utc)
if 'zona_id' not in st.session_state:
    st.session_state['zona_id'] = ""
if 'zona_nome' not in st.session_state:
    ...
    if scelta == "➕ Ingresso":
        ...
        with st.form(key=f"f_ingresso_{st.session_state['form_ingresso_ver']}"):
            if not st.session_state['zona_id']:
                ...
                st.info(f"📍 Zona: **{st.session_state['zona_nome']}**")
