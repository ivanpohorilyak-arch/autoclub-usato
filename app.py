...
def login_db(nome, pin):
    try: 
        res = (
            supabase
            .table("utenti")
            .select("nome, ruolo, can_consegna")
            .eq("nome", nome)   # ✅ Corretto: usa il parametro della funzione
            .eq("pin", pin)
            .eq("attivo", True)
            .limit(1)
            .execute()
...
def descrivi_modifiche(old, new):
    campi = {"marca_modello": "Marca/Modello", "colore": "Colore", "km": "KM", "numero_chiave": "Chiave"}
    modifiche = []
...
if st.session_state['user_autenticato'] is None:
        ...
        with st.form(key=f"f_ingresso_{st.session_state['form_ingresso_ver']}"): 
            ...
            if submit: 
                ...
                if n_chiave != 0 and n_chiave in occupate:
                    st.error(f"❌ La chiave {n_chiave} è già occupata da un'altra vettura in piazzale")
                    st.stop()
    ...
    # --- 9. SEZIONE RICERCA (Spostamento, Modifica, Consegna) --- 
    elif scelta == "🔍 Ricerca": 
        aggiorna_attivita() 
        ...
        if st.session_state["ricerca_attiva"]: 
            ...
            if len(risultati) > 1: 
                st.session_state["vettura_selezionata"] = st.selectbox("Seleziona vettura", risultati, format_func=lambda x: f"{x['targa']} | {x['marca_modello']} | Chiave {x['numero_chiave']}") 
            ...
            if v: 
                ...
                with st.expander("📜 Visualizza Storico Movimenti"): 
                    ...
                    if log.data: 
                        ...
                        df_log["Nota"] = df_log["dettaglio"].apply(lambda d: d.split("Nota:", 1)[1].strip() if "Nota:" in d else "")
                ...
                if st.session_state["azione_attiva"] == "spost": 
                    if not st.session_state.camera_attiva: st.warning("📷 Attiva lo Scanner QR dalla sidebar") 
                    else: 
                        nota_spost = st.text_area("Nota per lo spostamento (opzionale)") 
                        foto = st.camera_input("📷 Scanner QR Zona Destinazione") 
                        if foto: 
                            ...
                            if z_id: 
                                if st.button(f"➡️ SPOSTA IN {ZONE_INFO[z_id]}", use_container_width=True): 
                                    ...
                                    st.session_state["post_azione_msg"] = f"✅ Vettura spostata in **{ZONE_INFO[z_id]}**" 
                                    st.rerun() 
                            else: st.error("❌ QR non valido") 
                    ...
                    with st.form("f_mod_v"): 
                        ...
                        if st.form_submit_button("💾 SALVA MODIFICHE"): 
                            ...
                            registra_log(v["targa"], "Modifica", f"Modificati: {diff}" if diff else "Correzione dati", utente_attivo) 
                            st.session_state["post_azione_msg"] = "✅ Dati aggiornati" 
                            st.rerun() 

                elif st.session_state["azione_attiva"] == "cons": 
                    if not st.session_state.can_consegna: st.error("🔒 Non autorizzato") 
                    else: 
                        conferma = st.checkbox(f"Confermo consegna {v['targa']}") 
                        if st.button("🔴 ESEGUI CONSEGNA", disabled=not conferma, use_container_width=True): 
                            ...
                            st.session_state["post_azione_msg"] = "✅ Consegnata" 
                            st.rerun() 

    # --- ALTRE SEZIONI (Log, Export, Dashboard, etc.) --- 
    elif scelta == "📜 Log": 
        ...
        if res_all.data: st.dataframe(pd.DataFrame(res_all.data)[["nome", "ruolo", "attivo", "can_consegna"]], use_container_width=True) 
        ...
        with col1: 
            with st.form("add_user"): 
                ...
                c = st.checkbox("Consegna") 
                if st.form_submit_button("CREA"): 
                    supabase.table("utenti").insert({"nome": n, "pin": p, "ruolo": r, "attivo": True, "can_consegna": c}).execute() 
                    st.rerun()
