...
if st.session_state['user_autenticato'] is None:
    ...
    if scelta == "➕ Ingresso":
        ...
        attiva_cam = st.toggle("📸 Attiva Scanner Targa (OCR)")
        t_letta = ""
        if attiva_cam:
        ...
        with st.form("f_ingresso", clear_on_submit=True):
            ...
            if st.form_submit_button("REGISTRA VETTURA"):
                ...
                else: st.error("Targa, Marca e Modello sono obbligatori")

    # --- 2. RICERCA / SPOSTA ---
    elif scelta == "🔍 Ricerca/Sposta":
        st.subheader("Ricerca e Gestione Movimenti")
                    ...
                    for v in res.data:
                        with st.expander(f"🚗 {v['targa']} - {v['marca_modello']}", expanded=True):
                            ...
                            if c1.button("Conferma Spostamento", key=f"b_{v['targa']}"):
                                ...
                                registra_log(v['targa'], "Spostamento", f"Da {v['zona_attuale']} a {n_z}", utente_attivo)
                                st.rerun()
                            if c2.button("🔴 CONSEGNA (Uscita)", key=f"d_{v['targa']}"):
    ...
    # --- 3. VERIFICA ZONE ---
        ...
        occ = len(res.data)
        st.metric(f"Stato {z_sel}", f"{occ} / 100", delta=f"{100-occ} posti liberi")
        if res.data:
    ...
    # --- 4. EXPORT ---
        ...
        st.subheader("Esporta Database Attuale")
    ...
    # --- 5. LOG MOVIMENTI ---
        ...
        if auto_refresh:
            st.experimental_autorefresh(interval=10_000, key="log_refresh")
        
        c1, c2 = st.columns(2)
        f_targa = c1.text_input("Cerca Targa nei Log").upper().strip()
        f_user = c2.selectbox("Filtra per Operatore", ["Tutti"] + list(CREDENZIALI.keys()))
