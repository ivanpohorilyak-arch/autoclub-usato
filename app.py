...
if st.session_state['user_autenticato'] is None:
    ...
    if utenti:
        ...
        st.dataframe(
            df_ut[["nome", "ruolo", "attivo", "can_consegna"]],
            use_container_width=True
        )

    # Conta quanti admin attivi esistono
    ...
    if utenti:
        ...
        u_sel_nome = st.selectbox(
            "Seleziona utente",
            [u["nome"] for u in utenti]
        )
        ...
        with st.form("edit_user"):
            ...
            new_pin = st.text_input(
                "Nuovo PIN (vuoto per non cambiare)",
                type="password"
            )
            ...
            new_ruolo = st.selectbox(
                "Ruolo",
                ["operatore", "admin"],
                index=0 if ut_data["ruolo"] == "operatore" else 1
            )

            new_can_cons = st.checkbox(
                "Autorizzato alla CONSEGNA",
                value=ut_data.get("can_consegna", False)
            )

            new_attivo = st.checkbox(
                "Utente Attivo",
                value=ut_data["attivo"]
            )
            ...
            if salva:
                ...
                # 🔒 BLOCCO: admin non può auto-disattivarsi
                if (
                    u_sel_nome == utente_loggato
                    and ut_data["ruolo"] == "admin"
                    and not new_attivo
                ):
                    st.error("❌ Non puoi disattivare te stesso come admin")
                    st.stop()

                # 🛑 BLOCCO: non puoi togliere l’ultimo admin
                if (
                    ut_data["ruolo"] == "admin"
                    and new_ruolo != "admin"
                    and len(admin_attivi) <= 1
                ):
                    st.error("❌ Non puoi rimuovere l’ultimo admin attivo")
                    st.stop()
                ...
                if (
                    ...
                    st.error("❌ Non puoi disattivare l’ultimo admin attivo")
                    st.stop()
            ...
            if elimina:
                ...
                # 🔒 BLOCCO: non puoi eliminare te stesso
                if u_sel_nome == utente_loggato:
                    st.error("❌ Non puoi eliminare te stesso")
                    st.stop()

                # 🛑 BLOCCO: non puoi eliminare ultimo admin
                if ut_data["ruolo"] == "admin" and len(admin_attivi) <= 1:
                    st.error("❌ Non puoi eliminare l’ultimo admin attivo")
                    st.stop()
                ...
                st.success("🗑 Utente eliminato definitivamente")
                time.sleep(1)
                st.rerun()
